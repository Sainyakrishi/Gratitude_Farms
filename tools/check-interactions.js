...
const ok = (n, c, extra = '') => { console.log(`  ${c ? '✓' : '✗'} ${n}${extra ? '  — ' + extra : ''}`); if (!c) fails++; };
...
(async () => {
  ...
  {
    const p = await open('Home.dc.html', { width: 390, height: 844, isMobile: true });
  ...
  console.log('\nContact form');
  {
    const p = await open('Contact.dc.html');
    ...
    await p.$eval('#gf-email', e => { e.value = ''; });
    ...
    await p.click('button[type="submit"]');
    await settle(900);
    const sent = await p.$eval('body', b => b.innerText.includes('Almost there') || b.innerText.includes('enquiry received'));
    ok('valid submit reaches the success state', sent);
    ok('enquiry is handed to the mail client', !!mailto, mailto ? decodeURIComponent(mailto).slice(0, 58) + '…' : 'no mailto fired');
    await p.close();
  }
  {
    const p = await open('Contact.dc.html?service=Managed%20Farmland%20Service&audience=Corporates%20%26%20PSUs');
  ...
  console.log('\nService pages');
  {
    const p = await open('Managed-Farmland.dc.html');
  ...
  {
    const p = await open('Managed-Farmland.dc.html?audience=fpos');
  ...
  console.log('\nBlog');
  {
    const p = await open('Blog.dc.html');
    const all = await p.$$eval('a[href^="Blog-Post"]', a => a.length);
    const chips = await p.$$('.gf-chip');
    ok('category chips are rendered from the posts', chips.length >= 4, `${chips.length} chips`);
    await chips[1].click();
    await settle(400);
    const filtered = await p.$$eval('a[href^="Blog-Post"]', a => a.length);
  ...
  console.log('\nTree catalogue');
  {
    const p = await open('Medicinal-Plants-Trees.dc.html');
  ...
  console.log('\nSitewide');
  {
    ...
    ok('WhatsApp + call dock present on every page', dock.length === 2 && dock[0].includes('wa.me') && dock[1].startsWith('tel:'));
